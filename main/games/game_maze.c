...
static void generate(void)
{
    ...
    while (head < tail) {
        xf_point_t p = q[head++];
        if (dist[p.y][p.x] > dist[s.goal.y][s.goal.x]) {
            s.goal = p;
        }
        for (int d = 0; d < 4; ++d) {
            int nx = p.x + sx[d];
...
{
    (void)dt;
    if (!in->dir_changed || in->dir == XF_DIR_NONE) {
        return;
    }
    ...
    if (in->dir == XF_DIR_UP) {
        dy = -1;
    } else if (in->dir == XF_DIR_DOWN) {
        dy = 1;
    } else if (in->dir == XF_DIR_LEFT) {
        dx = -1;
    } else if (in->dir == XF_DIR_RIGHT) {
        dx = 1;
    }
...
static void render(void)
{
    ...
    int gx = s.goal.x - vx;
    int gy = s.goal.y - vy;
    if ((unsigned)gx < 8U && (unsigned)gy < 8U) {
